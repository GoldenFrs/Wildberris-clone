const cart = function(){
    ...
    const cartTotalCell = document.querySelector('.card-table__total');
    ...
    const total = (goods) => {
        if (!cartTotalCell) return;
        
        if (!goods || goods.length === 0) {
            cartTotalCell.textContent = '0$';
            return;
        }
        
        const totalSum = goods.reduce((sum, good) => sum + good.price * good.count, 0);
        cartTotalCell.textContent = `${totalSum}$`;
    };
